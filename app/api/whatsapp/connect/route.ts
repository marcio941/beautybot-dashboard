...
export async function GET() {
  try {
    ...
    const res = await fetch(`${EVOLUTION_URL}/instance/connect/${conta.whatsapp_instance}`, {
      method: "GET",
    ...
    return NextResponse.json({ base64: data?.qrcode?.base64 ?? null, code: data?.qrcode?.code ?? null });
  } catch (err: any) {
